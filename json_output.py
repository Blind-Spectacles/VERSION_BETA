...
def detect_objects_and_distance(camera_input):
    ...
    # Return JSON format output (list of objects with distances)
    return results, frame, boxes, classes, scores
...
def main():
    ...
    while True:
        results, frame, boxes, classes, scores = detect_objects_and_distance(cap)

        # Create a JSON output from the detected results
        json_output = json.dumps({"detected_objects": results}, indent=4)
